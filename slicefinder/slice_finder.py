...
class SliceFinder:
    ...
    def find_slice(self, k=50, epsilon=0.2, alpha=0.05, degree=3, risk_control=True, max_workers=1, name="Slice.p"):
        ...
        with open('./Slices/' + name,'wb') as handle:
    ...
    def evaluate_model(self, data, metric=log_loss):
        ...
        X = X.dropna()
        y = X['Label'].to_numpy()
        X = X.drop(['Label'], axis=1).reset_index(drop=True).to_numpy(copy=False)
